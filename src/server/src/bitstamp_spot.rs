use crate::{aggregator::Aggregator, grpc, level::Level, OrderbookSnapshot};
...
pub fn run_stream(source_id: usize, aggregator_arc: Arc<Mutex<Aggregator>>) {
    ...
    "#
        ...
        .expect("Unable to write message to Bitstamp websocket stream");
    ...
    loop {
        ...
        if let Ok(snapshot) = deserialization {
            if let Ok(mut aggregator) = aggregator_arc.lock() {
                let summary = aggregator.process(source_id, snapshot);
                grpc::publish_summary(summary);
            }
        }
    }
}
